...
MARKDOWN = ROOT / "GATE_EE/corpus_v1/source_batches/BATCH_003_GENERAL_APTITUDE.md"
MANIFEST = ROOT / "GATE_EE/corpus_v1/manifests/BATCH_003_MANIFEST.json"
HANDOFF = ROOT / "GATE_EE/corpus_v1/formatter_handoff/BATCH_003_FORMATTER_V2_HANDOFF.json"
FAMILIES = ROOT / "GATE_EE/corpus_v1/families/BATCH_003_FAMILIES.json"
GLOBAL_FAMILIES = ROOT / "GATE_EE/corpus_v1/families/FAMILY_REGISTRY.json"
TOPIC_MAP = ROOT / "GATE_EE/corpus_v1/TOPIC_MAP_V1.json"
FORMATTER_EVIDENCE = ROOT / "GATE_EE/corpus_v1/qualification/BATCH_003_FORMATTER_FINAL_EVIDENCE.json"
INDEPENDENT_QA = ROOT / "GATE_EE/corpus_v1/qualification/BATCH_003_INDEPENDENT_AI_QA.json"
CERTIFICATE = ROOT / "GATE_EE/corpus_v1/qualification/BATCH_003_PAPER_ELIGIBILITY_CERTIFICATE.json"
ADMISSION = ROOT / "GATE_EE/corpus_v1/manifests/BATCH_003_CORPUS_ADMISSION.json"
...
ga_domain = next((row for row in topic_map.get("domains", []) if row.get("code") == "GA"), {})
allowed_topics = set(ga_domain.get("topics", []))
allowed_subtopics = {
    "Basic English Grammar", "Vocabulary in Context", "Reading and Comprehension",
    "Narrative Sequencing", "Ratios and Percentages", "Data Interpretation",
    ...
    "Logic: Deduction and Induction", "Analogy and Numerical Relations",
    "Transformation of Shapes", "Paper Folding and Cutting",
    "Three-Dimensional Grouping", "Three-Dimensional Patterns",
}
...
for question in questions:
    ...
    if not re.fullmatch(r"TMB-GATE-EE-GA-\d{3}", str(qid)):
        errors.append(f"{qid}: invalid Batch 003 ID")
    ids.append(str(qid))
    families.append(str(question.get("family_id", "")))
    if question.get("exam") != "GATE_EE": errors.append(f"{qid}: wrong exam")
    if question.get("subject") != "General Aptitude": errors.append(f"{qid}: wrong subject")
    if question.get("topic") not in allowed_topics: errors.append(f"{qid}: out-of-map topic")
    ...
    if question.get("revision") != 1: errors.append(f"{qid}: Batch 003 source revision must be 1")
...
expected_ids = [f"TMB-GATE-EE-GA-{index:03d}" for index in range(1, 21)]
if len(questions) != 20: errors.append(f"expected 20 questions, found {len(questions)}")
if ids != expected_ids: errors.append("Batch 003 ID sequence mismatch")
if len(families) != len(set(families)): errors.append("duplicate family inside Batch 003")
if sum(question.get("_math_segment_count", 0) > 0 for question in questions) < 12:
    errors.append("Batch 003 must contain canonical LaTeX mathematics in at least 12 questions")
...
if certificate_exists and admission_exists:
    ...
    if manifest.get("status") != "PAPER_ELIGIBILITY_CERTIFIED_AND_ADMITTED":
        errors.append("certified Batch 003 manifest status is stale")
else:
    if manifest.get("status") != "READY_FOR_HUMAN_FINAL_QA":
        errors.append("pre-promotion Batch 003 manifest status mismatch")
...
if set(family_map) != set(families): errors.append("Batch 003 family file does not match source families")
for question in questions:
...
if certificate_exists and admission_exists:
    ...
    if found_admitted_family_map != expected_admitted_family_map:
        errors.append("admitted Batch 003 families do not match the global family registry")
else:
    if collision: errors.append(f"Batch 003 collides with admitted families: {sorted(collision)}")

markdown = MARKDOWN.read_text(encoding="utf-8")
if any(markdown.count(qid) != 1 for qid in expected_ids):
    errors.append("deterministic Markdown view does not contain every Batch 003 ID exactly once")
...
print("GATE EE PRODUCTION BATCH 003: PASSED")
print("Questions: 20")
print("Domain: General Aptitude")
print("IDs: TMB-GATE-EE-GA-001..020")
