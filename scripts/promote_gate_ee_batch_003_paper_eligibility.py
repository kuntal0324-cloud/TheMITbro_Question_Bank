...
from scripts.validate_gate_ee_batch_003_human_signoff import validate
...
BATCH2_CERTIFICATE = BASE / "qualification/BATCH_002_PAPER_ELIGIBILITY_CERTIFICATE.json"
BATCH2_ADMISSION = BASE / "manifests/BATCH_002_CORPUS_ADMISSION.json"
...
def block(errors: list[str]) -> None:
    print("BATCH 003 PAPER-ELIGIBILITY PROMOTION: BLOCKED")
    for error in errors:
        print("-", error)
    raise SystemExit(1)
...
def main() -> int:
    ...
    if len(batch_family_ids) != len(set(batch_family_ids)):
        preflight.append("Batch 003 family file contains duplicate family IDs")
    if sorted(batch_question_ids) != sorted(expected_ids):
        preflight.append("Batch 003 family file does not cover the canonical source exactly once")
    ...
    certificate = {
        "certificate_contract": "GATE_EE_BATCH003_PAPER_ELIGIBILITY_CERTIFICATE_V1",
        "batch_id": "BATCH_003",
    ...
    admission = {
        "manifest_contract": "GATE_EE_CORPUS_V1_BATCH003_ADMISSION_V1",
        "batch_id": "BATCH_003",
        "source_sha256": source_sha,
    ...
    summary.update(
        {
            ...
            "next_action": (
                "Use admitted questions only for controlled blueprint selection; continue Day-3 "
                "authoring with at least 27 additional blueprint-compatible selected-subject "
                "records before attempting Set 01 assembly."
            ),
        }
    )
    ...
    manifest.update(
        {
            ...
            "paper_eligible_count": len(approved),
            "qualification_note": (
                f"General Aptitude Batch 003. Strict Formatter: 20 PASS / 0 REVIEW / 0 invalid. Independent AI "
    ...
    batch_families["registry_version"] = "1.2.0"
            ...
            admitted_family_rows.append(
                {
                    ...
                    "admission_batch": "BATCH_003",
                }
            )
    family_registry["registry_version"] = "1.2.0"
    ...
    batch2_certificate = load(BATCH2_CERTIFICATE)
    batch2_admission = load(BATCH2_ADMISSION)
    total_eligible = (
        batch1_certificate.get("paper_eligible_count", 0)
        + batch2_certificate.get("paper_eligible_count", 0)
        + len(approved)
    )
    total_admitted = (
        batch1_admission.get("admitted_count", 0)
        + batch2_admission.get("admitted_count", 0)
        + len(approved)
    )
    progress["as_of"] = review_date
    progress["sprint"].update(
        {
            "current_day": 3,
            "state": "DAY_03_IN_PROGRESS",
        }
    )
    progress["day_2"]["state"] = "CLOSED_BELOW_TARGET"
    progress["day_3"] = {
        "target": 447,
        "drafts_produced": 0,
        "remaining": 447,
        "allocation": {
            "General Aptitude": {"target": 150, "produced": 0},
            "Engineering Mathematics": {"target": 150, "produced": 0},
            "Electromagnetic Fields": {"target": 147, "produced": 0},
        },
        "source_batches": [],
        "state": "IN_PROGRESS",
    }
    ...
    progress["next_gate"] = (
        "Author and qualify at least 27 additional blueprint-compatible selected-subject records, "
        "principally core EE; Set 01 remains blocked until its exact 65-question blueprint is complete."
    )
    ...
    print("BATCH 003 PAPER-ELIGIBILITY PROMOTION: PASSED")
