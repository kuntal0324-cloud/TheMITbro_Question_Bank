#!/usr/bin/env python3
"""Import a completed Batch 003 AcroForm into the canonical human-QA JSON."""
...
TEMPLATE = BASE / "review_manifests/BATCH_003_HUMAN_FINAL_QA.json"
...
def build_payload(pdf_path: Path) -> dict[str, Any]:
    ...
    if template.get("source_sha256") != sha256(SOURCE):
        errors.append("repository template is not locked to the current Batch 003 source")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert a completed Batch 003 fillable PDF into canonical human-QA JSON."
    )
    ...
    print("Next: python scripts/validate_gate_ee_batch_003_human_signoff.py")
