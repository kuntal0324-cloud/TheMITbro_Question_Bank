...
def main() -> int:
    ...
    if stage == "READY_FOR_HUMAN_FINAL_QA":
        if certificate_exists or admission_exists:
            errors.append("pre-promotion state must not contain a Batch 002 certificate or admission manifest")
        if candidate.get("paper_eligible_count") != 0:
            errors.append("pre-promotion Batch 002 paper eligibility must be zero")
        if candidate.get("human_final_qa") != "PENDING":
        ...
        if manifest.get("status") != "READY_FOR_HUMAN_FINAL_QA" or manifest.get("paper_eligible_count") != 0:
            errors.append("pre-promotion Batch 002 manifest state mismatch")
        if batch_families.get("admission_status") != "DRAFT_NOT_ADMITTED":
            errors.append("pre-promotion Batch 002 families must not be admitted")
        ...
        if global_ids.intersection(expected_ids):
            errors.append("pre-promotion global family registry contains Batch 002 IDs")
        ...
        if not certificate_exists or not admission_exists:
            ...
            if certificate.get("certificate_contract") != "GATE_EE_BATCH002_PAPER_ELIGIBILITY_CERTIFICATE_V1":
                errors.append("certificate contract mismatch")
            if certificate.get("batch_id") != "BATCH_002": errors.append("certificate batch mismatch")
            ...
            if admission.get("manifest_contract") != "GATE_EE_CORPUS_V1_BATCH002_ADMISSION_V1":
                errors.append("admission contract mismatch")
            if admission.get("batch_id") != "BATCH_002": errors.append("admission batch mismatch")
            ...
            if manifest.get("status") != "PAPER_ELIGIBILITY_CERTIFIED_AND_ADMITTED":
                errors.append("certified Batch 002 manifest status mismatch")
            if manifest.get("paper_eligible_count") != len(approved):
                errors.append("Batch 002 manifest eligible count mismatch")
            if batch_families.get("admission_status") != "ADMITTED_TO_CORPUS_V1":
                errors.append("Batch 002 family admission state mismatch")
            ...
            if found_family_map != expected_family_map:
                errors.append("global family registry does not match admitted Batch 002 families")
    else:
        errors.append(f"unsupported Batch 002 qualification stage: {stage!r}")

    if errors:
        print("BATCH 002 PAPER-ELIGIBILITY CERTIFICATION: BLOCKED")
        for error in errors:
            print("-", error)
        return 1
    ...
    if stage == "READY_FOR_HUMAN_FINAL_QA":
        if human.get("final_decision") == "APPROVE_REVIEWED_RESULTS":
            print("BATCH 002 PAPER-ELIGIBILITY CERTIFICATION STATE: READY_FOR_PROMOTION")
            print(f"Human-approved: {len(approved)}")
            print(f"Held for revision: {len(revise)}")
            print(f"Rejected: {len(reject)}")
        else:
            print("BATCH 002 PAPER-ELIGIBILITY CERTIFICATION STATE: READY_FOR_HUMAN_FINAL_QA")
