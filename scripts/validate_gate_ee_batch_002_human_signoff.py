...
def validate(require_complete: bool = False) -> tuple[list[str], list[str], list[str], list[str], str]:
    ...
    if human.get("signoff_contract") != "GATE_EE_BATCH_002_HUMAN_FINAL_QA_V1":
        errors.append("human-signoff contract mismatch")
    if human.get("batch_id") != "BATCH_002": errors.append("human-signoff batch mismatch")
    ...
    if stage not in ALLOWED_STAGES: errors.append("Batch 002 is not at a valid human-final-QA state")
...
def main() -> int:
    ...
    if human.get("final_decision") == "PENDING":
        ...
        print("GATE EE BATCH 002 HUMAN FINAL QA SIGNOFF: PASSED")
        print(f"Source SHA-256: {source_sha}")
        print(f"PASS: {len(approved)}")
        print(f"REVISE: {len(revise)}")
        print(f"REJECT: {len(reject)}")
