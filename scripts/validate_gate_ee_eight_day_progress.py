...
if len(daily_targets) != 8 or sum(daily_targets) != 3575: errors.append("eight-day daily targets must sum to 3,575")
...
try:
    ...
    errors.append("invalid sprint calendar dates")
...
if set(opening_batches + day1_batches) - set(records): errors.append("ledger references an unknown source batch")
...
program_count = len(all_questions)
...
admission = load(BASE / "manifests/BATCH_001_CORPUS_ADMISSION.json")
certificate = load(BASE / "qualification/BATCH_001_PAPER_ELIGIBILITY_CERTIFICATE.json")
admitted = admission.get("admitted_count", 0)
eligible = certificate.get("paper_eligible_count", 0)
if admitted != len(admission.get("admitted_question_ids", [])): errors.append("Batch 001 admission count mismatch")
if eligible != len(certificate.get("approved_question_ids", [])): errors.append("Batch 001 certificate count mismatch")
if opening.get("paper_eligible") != eligible or opening.get("corpus_admitted") != admitted:
    errors.append("opening certified inventory mismatch")
if program.get("paper_eligible") != eligible or program.get("corpus_admitted") != admitted:
    errors.append("program certified inventory mismatch")

formatter_passed = load(BASE / "qualification/BATCH_001_FORMATTER_FINAL_EVIDENCE.json").get("formatter_pass_count", 0)
batch2_evidence = BASE / "qualification/BATCH_002_FORMATTER_FINAL_EVIDENCE.json"
if batch2_evidence.exists():
    formatter_passed += load(batch2_evidence).get("formatter_pass_count", 0)
if program.get("formatter_passed") != formatter_passed: errors.append("program Formatter-pass count mismatch")
if opening.get("formatter_passed") != 20: errors.append("opening Formatter-pass count mismatch")
if opening.get("human_final_qa_passed") != eligible or program.get("human_final_qa_passed") != eligible:
    errors.append("human-final-QA count mismatch")
...
print(f"Formatter-passed: {formatter_passed}")
print(f"Paper-eligible/admitted: {eligible}/{admitted}")
