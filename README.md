...
The exact selection is now frozen in `GATE_EE/corpus_v1/paper_manifests/GATE_EE_SET_01_V1.json`, with a checksum-bound Formatter review handoff and deterministic question, solution and mobile-fillable whole-paper QA PDFs. This is a **review checkpoint**, not a release: the complete rendered paper still needs named-human technical and visual QA. The reviewer must also reconfirm the accurate qualification metadata because the historical Batch 001 signoff records a different qualification from Batches 002–005. Catalog publication, sale and release remain blocked.

Run `python scripts/validate_gate_ee_set01_review_artifacts.py` to verify all 65 source/evidence bindings and committed PDFs. After a named reviewer completes the issued form, import it with `python scripts/import_gate_ee_set01_human_qa_pdf.py` and enforce completion with `python scripts/validate_gate_ee_set01_human_signoff.py --require-complete`.
