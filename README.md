...
RC1 now provides clean learner-facing question, solution and combined learner-pack PDFs plus a 14-field mobile-fillable exact-artifact authorization form. Run `python scripts/validate_gate_ee_set01_release_candidate.py` to verify its checksums, fonts, learner-copy hygiene and exact question/solution concatenation. This is still a **candidate**, not a release: complete `output/pdf/GATE_EE_SET_01_RELEASE_AUTHORIZATION_MOBILE_FILLABLE.pdf`, save it as `GATE_EE_SET_01_RELEASE_AUTHORIZATION_COMPLETED.pdf`, import it with `python scripts/import_gate_ee_set01_release_authorization_pdf.py`, then validate with `python scripts/validate_gate_ee_set01_release_authorization.py --require-complete`. Pricing, storefront activation and sale require a later explicit promotion and remain blocked.
